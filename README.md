...
5. **팝업 페이지 캐시**: `ai-report.html`을 수정하면 `js/20-ai-report.js`의 `AI_REPORT_PAGE_VERSION` 값을 반드시 올립니다. 팝업은 메인 페이지와 별개 문서이므로, 메인 창에서 Ctrl+Shift+R을 눌러도 팝업 캐시는 갱신되지 않습니다. 이 쿼리 문자열이 유일한 갱신 수단입니다.
