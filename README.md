...
빌드 도구 없이 브라우저에서 바로 도는 **클래식 스크립트** 방식입니다. 원래 한 파일이던 `app.js`(약 5,300줄)를 기능별로 나눴으며, 모든 모듈은 하나의 전역(window) 스코프를 공유합니다. 따라서 `index.html`에 나열된 `<script>` **로딩 순서(01 → 17)를 반드시 유지**해야 하며, 순서를 바꾸면 동작이 깨질 수 있습니다.
...
```
index.html            진입점. js/01~17을 순서대로 로드하고 차트 줌을 전역 설정
